...
- **injectCss** *(Default: `true`)*: `boolean` If `false`, the factory will no try to inject the default CSS for the map into the `<head>` element. You also have to set `window.MAPBOX_SKIP_CSS_INJECTION` to `true` before importing the package to deactivate the legacy CSS injection.
